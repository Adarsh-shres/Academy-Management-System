...
export default function NotificationBell() {
  ...
  const getFilterClass = (f: FilterType) => `px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider transition-colors ${
    filter === f ? 'bg-[#232529] text-white' : 'bg-[#F6F8FB] text-[#7c8697] hover:bg-[#E1E6EE]'
  }`;
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {showToast && latestNotification && (
        <div className="fixed top-20 right-6 bg-white border border-[#E1E6EE] shadow-[0_10px_40px_rgba(36,37,41,0.15)] rounded-[12px] p-4 w-80 z-[100] animate-in slide-in-from-right-10 fade-in duration-300">
          <div className="flex justify-between items-start">
            <div className="flex gap-3">
              <div className="w-8 h-8 rounded-full bg-[#F6F8FB] flex items-center justify-center shrink-0">
                <svg className="w-4 h-4 text-[#232529]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                </svg>
              </div>
              <div>
                <h4 className="text-[13px] font-bold text-[#232529]">{latestNotification.title}</h4>
                <p className="text-[12px] text-[#7c8697] mt-0.5 line-clamp-2">{latestNotification.message}</p>
              </div>
            </div>
            <button onClick={() => setShowToast(false)} className="text-[#a0a8b5] hover:text-[#232529]">
      ...
      <button
        ...
        className={`relative p-2 rounded-full transition-colors ${isOpen ? 'bg-[#F6F8FB] text-[#232529]' : 'hover:bg-[#F6F8FB] text-[#7c8697] hover:text-[#232529]'}`}
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute top-1.5 right-1.5 flex h-2.5 w-2.5">
            ...
            <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-[#232529] border-2 border-white"></span>
          </span>
        )}
      </button>
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute right-0 mt-2 w-[360px] bg-white rounded-[12px] shadow-[0_15px_40px_rgba(36,37,41,0.12)] border border-[#E1E6EE] overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="p-4 border-b border-[#F6F8FB] bg-white">
            <div className="flex justify-between items-center mb-3">
              <h3 className="font-extrabold text-[#232529] text-[16px] tracking-tight">Notifications</h3>
              {unreadCount > 0 && (
          ...
          <div className="max-h-[340px] overflow-y-auto bg-[#faf8fc]">
            {filteredNotifications.length === 0 ? (
              <div className="p-8 text-center flex flex-col items-center gap-3">
                <div className="w-12 h-12 rounded-full bg-[#F6F8FB] flex items-center justify-center text-[#a0a8b5]">
              ...
              <div className="flex flex-col divide-y divide-[#E1E6EE]">
                {filteredNotifications.slice(0, 10).map((notif) => (
                  <div 
                    key={notif.id}
                    className={`p-4 transition-colors ${!notif.is_read ? 'bg-white border-l-[3px] border-l-[#232529]' : 'hover:bg-white border-l-[3px] border-l-transparent'}`}
                  >
                      ...
                      <div className="flex flex-col items-end gap-2 shrink-0">
                        {!notif.is_read && (
                          <button 
                            ...
                            className="w-2.5 h-2.5 rounded-full bg-[#232529] ring-4 ring-[#F6F8FB]"
                            title="Mark as read"
                          />
                        )}
                      </div>
          ...
          <div className="border-t border-[#E1E6EE] p-3 bg-white">
            <Link 
              to={historyPath} 
              onClick={() => setIsOpen(false)}
              className="flex items-center justify-center gap-2 w-full py-2 text-[12px] font-extrabold text-[#232529] hover:bg-[#F6F8FB] rounded-[8px] transition-colors uppercase tracking-wider"
            >
              View All History
