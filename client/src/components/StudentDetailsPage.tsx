  ...
  return (
    <div className="flex flex-col gap-6 md:gap-8 pb-10">
      <div className="flex items-center gap-4">
        <button 
          onClick={() => navigate(-1)}
          className="w-10 h-10 flex items-center justify-center bg-[#f3eff7] border border-[#e2d9ed] rounded-sm hover:bg-[#6a5182] hover:text-white transition-colors text-[#6a5182]"
        >
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/></svg>
        </button>
        <div>
          <h1 className="text-[28px] font-extrabold text-[#4b3f68] tracking-tight">Student Details</h1>
          <p className="text-[14px] text-[#64748b] mt-1">Viewing information for {student.firstName} {student.lastName}</p>
        </div>
      </div>

      <div className="bg-white rounded-sm border border-[#e2e8f0] p-6 md:p-8 shadow-sm">
        <h2 className="text-[18px] font-bold text-[#4b3f68] mb-6 border-b border-[#e2e8f0] pb-4">Personal & Academic Information</h2>
        
        <div className="flex flex-col md:flex-row gap-8">
          
          {/* Photo Section */}
          <div className="flex flex-col items-center gap-3 shrink-0">
            <div className="w-32 h-40 bg-[#f1f5f9] border border-[#e2e8f0] rounded-sm flex items-center justify-center overflow-hidden shrink-0">
            ...
            <span className={`px-3 py-1.5 rounded-sm text-[12px] font-bold ${
...
function DetailItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col gap-1.5 border-l-[3px] border-[#f3eff7] pl-3">
