...
const TeacherInstitutionalCalendar = () => {
  const [currentDate, setCurrentDate] = useState(new Date(2026, 3, 1)); // Default set to April 2026 or current
  ...
  // Mocks based strictly on prompt
  ...
  const getDayStyle = (day: number, index: number) => {
    ...
    const isWeekend = colIndex === 5 || colIndex === 6; // SAT or SUN

    if (isWeekend) return "text-[#ef4444] hover:bg-[#ffe4e6]"; // Holiday
    if (isAcademic(day)) return "text-[#3b82f6] hover:bg-[#dbeafe]"; // Academic
    if (isEvent(day)) return "text-[#f59e0b] hover:bg-[#fef3c7]"; // Event
    return "text-[#475569] hover:bg-[#f3eff7]"; // Default
  };
