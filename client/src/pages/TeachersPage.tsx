...
const SCHEDULE = [
  ...
  { day: 'Wednesday', time: '11:00 – 12:00', course: 'PHY-101 Intro to Physics',  room: 'Hall C-08' },
  { day: 'Friday',    time: '09:00 – 10:30', course: 'PHY-301 Quantum Mechanics', room: 'Hall A-12' },
...
const ACTIVITIES = [
  ...
  { id: 4, text: 'Sent announcement: Lab session rescheduled',      time: '2 days ago',   icon: '📢' },
  { id: 5, text: 'Updated course syllabus for PHY-101',             time: '3 days ago',   icon: '📚' },
];
      ...
      <div className="flex flex-col gap-6 md:gap-8 pb-10">
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className="w-10 h-10 flex items-center justify-center bg-white border border-[#e2e8f0] rounded-lg hover:bg-[#f8fafc] transition-colors text-[#64748b] cursor-pointer"
          >
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/></svg>
          </button>
          <div>
            <h1 className="text-[28px] font-extrabold text-[#0d3349] tracking-tight">Teacher Profile</h1>
            <p className="text-[14px] text-[#64748b] mt-1">Viewing details for {t.name}</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-[#e2e8f0] rounded-2xl p-6 md:p-8 shadow-sm">
          <div className="flex flex-col sm:flex-row gap-6">
            ...
            <div className="flex flex-col items-center gap-3 shrink-0">
              <div className={`w-[100px] h-[100px] rounded-2xl bg-gradient-to-br ${t.avatarGradient} text-white text-[32px] font-bold flex items-center justify-center shrink-0`}>
                {t.initials}
              </div>
              <span className={`inline-block px-3 py-1 rounded-full text-[11px] font-bold tracking-[0.02em] ${STATUS_STYLES[t.status]}`}>
                {t.status}
              </span>
            </div>
            {/* Info + Edit */}
            <div className="flex-1 flex flex-col justify-between min-w-0">
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div>
                  <h2 className="font-sans text-[22px] font-extrabold text-[#0d3349] tracking-tight leading-tight">{t.name}</h2>
                  <p className="text-[13px] text-[#64748b] mt-1">{t.subject}</p>
                </div>
              ...
              <div className="flex flex-wrap gap-x-10 gap-y-4 mt-5">
                <DetailField label="Employee ID" value={t.employeeId} />
                <DetailField label="Department" value={t.department} />
                <DetailField label="Specialization" value={t.subject} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-[18px]">
          ...
          <StatCard
            ...
            value={String(t.upcomingSessions)}
