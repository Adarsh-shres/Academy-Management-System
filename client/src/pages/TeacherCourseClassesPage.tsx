    ...
    <div className="app-shell flex h-screen font-sans overflow-hidden">
        ...
        <div className="flex-1 p-6 md:p-8 flex flex-col min-w-0 max-w-[1400px] mx-auto w-full">
          ...
          <div className="mb-8">
            <button
              ...
              className="inline-flex items-center gap-2 text-[13px] font-semibold text-[#4B5563] hover:text-[#232529] mb-4 cursor-pointer transition-colors"
            >
              <ChevronLeft size={16} />
              Back to My Courses
            </button>
            <h1 className="text-[26px] font-extrabold text-[#232529] tracking-tight">{course?.name || 'Course'} Classes</h1>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map(i => <div key={i} className="animate-pulse bg-white h-[200px] rounded-sm border border-[#E1E6EE]"></div>)}
        </div>
      ) : classes.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {classes.map((cls, idx) => {
            ...
            return (
              <div 
                ...
                className="bg-white rounded-sm border border-[#E1E6EE] shadow-[0_10px_28px_rgba(36,37,41,0.06)] flex flex-col hover:border-[#CCD4E0] hover:shadow-md transition-all cursor-pointer transform hover:-translate-y-1"
              >
                <div className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    <span className="bg-[#f1f5f9] text-[#232529] rounded-sm px-2.5 py-1 text-[11.5px] font-bold tracking-wide border border-[#e2e8f0]">
                      {course?.course_code || `CRS-${idx + 1}`}
                    </span>
                  </div>

                  <h3 className="text-[18px] font-bold text-[#232529] mb-2 leading-tight">{course?.name} - {cls.name || `Section ${idx + 1}`}</h3>
                  <p className="text-[13px] font-medium text-[#64748b] flex items-center gap-3">
                    ...
                    <span className="flex items-center gap-1.5 text-[#4B5563] font-semibold">{courseRoom}</span>
                  </p>
        ...
        <div className="flex flex-col items-center justify-center py-20 px-4 bg-white border border-dashed border-[#E1E6EE] rounded-sm w-full text-center">
