...
export default function TeacherClassDetailPage() {
  ...
  const [quizSubmissionsError, setQuizSubmissionsError] = useState<
    string | null
  >(null);
  ...
  const loadQuizSubmissions = async (quizId: string) => {
    ...
    try {
      ...
      if (studentIds.length > 0) {
        ...
        if (usersError) {
          console.warn(
            "Failed to load quiz submission student profiles",
            usersError,
          );
        } else {
          usersById = new Map(
            (usersData || []).map((student: any) => [student.id, student]),
          );
        }
      }
      ...
      setQuizSubmissionsError(
        err.message || "Failed to load quiz submissions.",
      );
      setQuizSubmissions([]);
    } finally {
      setIsLoadingSubmissions(false);
    ...
    try {
      ...
      const { error: quizError } = await supabase
        ...
        .update({
          ...
          time_limit_minutes: quizTimeLimit
            ? Math.round(Number(quizTimeLimit))
            : null,
      ...
      showToast(
        publish
          ...
          : "Quiz updated and saved as draft.",
      );
      resetQuizForm();
      setIsEditingQuiz(false);
      setEditingQuizId(null);
    ...
    try {
      ...
      showToast(
        publish
          ...
          : "Quiz saved as draft. It is not visible to students yet.",
      );
      resetQuizForm();
      loadQuizzes();
    } catch (err: any) {
  ...
  return (
    <div className="flex h-screen bg-main-bg font-sans overflow-hidden">
        ...
        <div className="flex-1 p-6 md:p-8 flex flex-col min-w-0 max-w-[1200px] mx-auto w-full">
          <div className="bg-white rounded-md border border-[#e7dff0] shadow-[0_10px_28px_rgba(57,31,86,0.06)] overflow-hidden mb-6">
            ...
            <div className="p-6 md:p-8 flex flex-col md:flex-row justify-between md:items-end gap-4">
              <div>
                ...
                <h2 className="text-[28px] font-extrabold text-[#4b3f68] mb-1 leading-tight tracking-tight">
              ...
              <div className="bg-[#fbf8fe] border border-[#e7dff0] p-4 rounded-sm flex items-center justify-between gap-6 md:w-auto w-full">
                <div>
                  ...
                  <p className="text-[22px] font-extrabold text-[#6a5182] leading-none">
                    {students.length}
                  </p>
                </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-[#fbf8fe] border-b border-[#e7dff0]">
                      <th className="py-4 px-6 text-[11px] font-bold text-[#64748b] uppercase tracking-wider w-[80px]">
                        S.No
                      </th>
                      <th className="py-4 px-6 text-[11px] font-bold text-[#64748b] uppercase tracking-wider">
                        Student Name
                      </th>
                      <th className="py-4 px-6 text-[11px] font-bold text-[#64748b] uppercase tracking-wider">
                        Institutional Email
                  ...
                  <tbody className="divide-y divide-[#e7dff0]">
                    {students.map((student, idx) => (
                      <tr
                        key={student.id}
                        className="hover:bg-[#fbf8fe]/50 transition-colors"
                      >
                        <td className="py-4 px-6 text-[13px] font-semibold text-[#64748b]">
                          {(idx + 1).toString().padStart(2, "0")}
                        </td>
                        <td className="py-4 px-6 text-[14px] font-bold text-[#4b3f68]">
                          {student.name}
                        </td>
                        <td className="py-4 px-6 text-[13.5px] text-[#64748b]">
                          {student.email}
                        </td>
                      </tr>
                    ))}
                      ...
                      <tr>
                        <td colSpan={3} className="py-16 text-center">
                          <div className="flex flex-col items-center">
                            ...
                            <p className="text-[14px] font-semibold text-[#4b3f68]">
                              No Active Enrollments
                            </p>
                            <p className="text-[13px] text-[#64748b] mt-1">
                              There are currently no students mapped to this
                              Class ID.
                            </p>
                          </div>
                        </td>
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-[#fbf8fe] border-b border-[#e7dff0]">
                          ...
                          <th className="py-4 px-6 text-[11px] font-bold text-[#64748b] uppercase tracking-wider text-center">
                            Status
                          </th>
                          <th className="py-4 px-6 text-[11px] font-bold text-[#64748b] uppercase tracking-wider text-right">
                            Action
                          </th>
                        </tr>
                      </thead>
                          ...
                          >
                            ...
                            <td className="py-4 px-6">
                              ...
                              {assignment.description && (
                                <p className="text-[12px] text-[#94a3b8] mt-0.5 truncate max-w-[300px]">
                                  {assignment.description}
                                </p>
                              )}
                            </td>
              ...
              {selectedAssignment && (
                <div className="flex flex-col gap-4">
                  <div className="bg-[#fbf8fe] border border-[#e7dff0] rounded-md p-4">
                    <p className="text-[15px] font-extrabold text-[#4b3f68]">
                    ...
                    assignmentSubmissions.length > 0 && (
                      <div className="px-5 py-3 bg-white rounded-md border border-[#e7dff0] shadow-[0_1px_4px_rgba(57,31,86,0.02)] flex gap-6 text-[13px] font-semibold text-[#64748b]">
                        <div className="flex items-center gap-1.5">
                          <span className="w-2 h-2 rounded-full bg-[#94a3b8] block"></span>{" "}
                          Total Students:{" "}
                  ...
                  <div className="bg-white rounded-md border border-[#e7dff0] overflow-hidden shadow-[0_10px_28px_rgba(57,31,86,0.06)]">
                    {isLoadingSubmissions ? (
                      ...
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="bg-[#fbf8fe] border-b border-[#e7dff0]">
                            <th className="py-4 px-6 text-[11px] font-bold text-[#64748b] uppercase tracking-wider w-[60px]">
                              #
                            </th>
                            <th className="py-4 px-6 text-[11px] font-bold text-[#64748b] uppercase tracking-wider">
                              Student
                              ...
                              >
                                ...
                                <td className="py-4 px-6 text-right whitespace-nowrap">
                                  {sub.file_url ? (
                                    ...
                                    <span className="text-[#94a3b8] mr-5">
                                      —
                                    </span>
                                  )}
                                  <button
            ...
            <div className="bg-white rounded-md border border-[#e7dff0] shadow-[0_10px_28px_rgba(57,31,86,0.06)] p-6 md:p-8 animate-fade-in">
              <div className="mb-6 border-b border-[#e7dff0] pb-4">
                <h3 className="text-[18px] font-extrabold text-[#4b3f68]">
                  Push Notification to {course.name}
                </h3>
                <p className="text-[13px] text-[#64748b] mt-1">
                  Sending to:{" "}
                  <strong className="text-[#4b3f68]">
                  ...
                  ({course.name}) — {students.length} enrolled students.
                </p>
              </div>
          ...
          {activeSubTab === "quizzes" && (
            <div className="flex flex-col gap-6 animate-fade-in">
              {toast && (
                <div
                  className={`fixed bottom-6 right-6 z-50 flex items-start gap-3 px-5 py-4 rounded-md shadow-xl border max-w-[380px] animate-fade-in ${
                    toast.type === "success"
                ...
                >
                  ...
                  <button
                    ...
                    className="ml-auto text-[#94a3b8] hover:text-[#64748b] text-[14px] cursor-pointer"
                  >
                    ✕
                  </button>
                </div>
              ...
              {!showQuizForm && !selectedQuiz && (
                <div className="flex justify-between items-center">
                  <p className="text-[13.5px] text-[#64748b] font-medium">
                    {quizzes.length} quiz{quizzes.length !== 1 ? "zes" : ""} in
                    this class
                  </p>
                  <button
                ...
                <div className="bg-white rounded-md border border-[#e7dff0] shadow-[0_10px_28px_rgba(57,31,86,0.06)] p-6 md:p-8">
                  <div className="flex justify-between items-center mb-6 border-b border-[#e7dff0] pb-4">
                    ...
                    <button
                      ...
                      className="text-[#64748b] hover:text-[#4b3f68] text-[13px] font-semibold cursor-pointer"
                    >
                      ✕ Cancel
                    </button>
                  </div>
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          onChange={(e) =>
                            setQuizAccessCode(e.target.value.toUpperCase())
                          }
                        ...
                        <button
                          ...
                          className="px-4 py-2 bg-[#f3eff7] text-[#6a5182] text-[13px] font-bold rounded-sm whitespace-nowrap hover:bg-[#e7dff0] transition-colors border border-[#d8c8e9] cursor-pointer"
                        >
                          Generate Code
                        </button>
                      </div>
                  ...
                  <div className="flex flex-col gap-5 mb-6">
                    <div className="flex justify-between items-center">
                      <h4 className="text-[14px] font-extrabold text-[#4b3f68] uppercase tracking-wide">
                        Questions
                      </h4>
                      <button
                        onClick={addQuestion}
                    ...
                    {questions.map((q, qIdx) => (
                      <div
                        key={qIdx}
                        className="bg-[#fbf8fe] border border-[#e7dff0] rounded-md p-5 flex flex-col gap-4"
                      >
                        <div className="flex justify-between items-start gap-3">
                          <span className="text-[12px] font-bold text-[#6a5182] uppercase tracking-wide">
                            Q{qIdx + 1}
                          </span>
                          {questions.length > 1 && (
                            <button
                              onClick={() => removeQuestion(qIdx)}
                              className="text-[#94a3b8] hover:text-red-500 text-[12px] font-bold cursor-pointer transition-colors"
                            >
                              🗑 Remove
                            </button>
                          )}
                        ...
                        {q.question_type === "mcq" && (
                          <div className="flex flex-col gap-2">
                            ...
                            {["A", "B", "C", "D"].map((letter, oIdx) => (
                              ...
                              >
                                <span className="text-[12px] font-bold text-[#6a5182] w-5">
                                  {letter}.
                                </span>
                                <input
                                  type="text"
                  ...
                  <div className="flex gap-3 pt-2 border-t border-[#e7dff0]">
                    <button
                      onClick={() =>
                        isEditingQuiz ? updateQuiz(false) : saveQuiz(false)
                      }
                    ...
                    <button
                      onClick={() =>
                        isEditingQuiz ? updateQuiz(true) : saveQuiz(true)
                      }
                      disabled={isSavingQuiz}
                ...
                <div className="bg-white rounded-md border border-[#e7dff0] shadow-[0_10px_28px_rgba(57,31,86,0.06)] p-6 md:p-8">
                  <div className="flex justify-between items-start mb-6 border-b border-[#e7dff0] pb-4">
                    <div>
                      <button
                        ...
                        className="text-[#64748b] hover:text-[#4b3f68] text-[13px] font-semibold cursor-pointer mb-2"
                      >
                        ← Back to Quiz
                      </button>
                      <h3 className="text-[18px] font-extrabold text-[#4b3f68]">
                      ...
                      <div className="flex gap-4 mt-2 text-[12px] text-[#64748b] font-medium items-center">
                        {selectedQuiz.due_date && (
                          <span>
                            Due:{" "}
                            {new Date(selectedQuiz.due_date).toLocaleString()}
                          </span>
                        )}
                        {selectedQuiz.time_limit_minutes && (
                          <span>⏱ {selectedQuiz.time_limit_minutes} min</span>
                        )}
                        <span>
                  ...
                  <div className="flex flex-col gap-4">
                    {quizQuestions.map((q, idx) => (
                      <div
                        key={q.id}
                        className="bg-[#fbf8fe] border border-[#e7dff0] rounded-md p-5"
                      >
                        <div className="flex justify-between items-start mb-3">
                          <p className="text-[14px] font-bold text-[#4b3f68]">
                            Q{idx + 1}. {q.question_text}
                          </p>
                          <span className="text-[11px] font-bold text-[#6a5182] bg-[#f3eff7] px-2 py-0.5 rounded-sm border border-[#d8c8e9]">
                        ...
                        {q.question_type === "mcq" && q.options && (
                          <div className="flex flex-col gap-1.5 mt-2">
                            {q.options.map((opt: string, oIdx: number) => (
                              <div
                                key={oIdx}
                                className={`flex items-center gap-2 px-3 py-2 rounded-sm text-[13px] ${String(oIdx) === q.correct_answer ? "bg-[#dcfce7] text-[#16a34a] font-bold border border-[#86efac]" : "bg-white border border-[#e7dff0] text-[#64748b]"}`}
                              >
                                <span className="font-bold">
                                  {["A", "B", "C", "D"][oIdx]}.
                                </span>{" "}
                                {opt}
                                {String(oIdx) === q.correct_answer && (
                        ...
                        {q.question_type === "true_false" && (
                          <div className="flex gap-2 mt-2">
                            {["true", "false"].map((val) => (
                              <div
                                key={val}
                                className={`px-4 py-2 rounded-sm text-[13px] font-bold border capitalize ${val === q.correct_answer ? "bg-[#dcfce7] text-[#16a34a] border-[#86efac]" : "bg-white border-[#e7dff0] text-[#64748b]"}`}
                              >
                  ...
                  <div className="mt-8 border-t border-[#e7dff0] pt-6">
                      ...
                      <div className="flex items-center gap-3">
                        {quizSubmissions.length > 0 && (
                          <div className="flex items-center gap-4 text-[12px] font-bold text-[#64748b]">
                            <span>
                              {quizSubmissions.length} Submission
                              {quizSubmissions.length !== 1 ? "s" : ""}
                            </span>
                    ...
                    {quizSubmissionsError ? (
                      <div className="flex flex-col items-center justify-center py-12 px-4 bg-[#fff7f7] border border-dashed border-red-200 rounded-md text-center">
                        ...
                        <p className="text-[13px] text-[#64748b] mt-1 max-w-[680px]">
                          {quizSubmissionsError}
                        </p>
                      </div>
                    ) : isLoadingSubmissions ? (
                      <div className="flex flex-col gap-3">
                        {[1, 2, 3].map((i) => (
                          <div
                            key={i}
                            className="animate-pulse bg-[#fbf8fe] h-[60px] rounded-md border border-[#e7dff0]"
                          />
                        ))}
                      </div>
                    ) : quizSubmissions.length === 0 ? (
                      ...
                      <div className="bg-white rounded-md border border-[#e7dff0] overflow-hidden shadow-[0_10px_28px_rgba(57,31,86,0.06)]">
                        <table className="w-full text-left border-collapse">
                          <thead>
                            <tr className="bg-[#fbf8fe] border-b border-[#e7dff0]">
                              <th className="py-3 px-5 text-[11px] font-bold text-[#64748b] uppercase tracking-wider w-[50px]">
                                #
                              </th>
                              <th className="py-3 px-5 text-[11px] font-bold text-[#64748b] uppercase tracking-wider">
                                Student
                              </th>
                              <th className="py-3 px-5 text-[11px] font-bold text-[#64748b] uppercase tracking-wider text-center">
                                Score
                              </th>
                              <th className="py-3 px-5 text-[11px] font-bold text-[#64748b] uppercase tracking-wider text-center">
                                Percentage
                          ...
                          <tbody className="divide-y divide-[#e7dff0]">
                            {quizSubmissions.map((sub, idx) => {
                              ...
                              const mins = sub.time_taken_seconds
                                ? Math.floor(sub.time_taken_seconds / 60)
                                : null;
                              const secs = sub.time_taken_seconds
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td className="py-3 px-5 text-center">
                                    <span
                                      className={`text-[13px] font-extrabold ${sub.percentage >= 75 ? "text-[#16a34a]" : sub.percentage >= 50 ? "text-[#ca8a04]" : "text-red-500"}`}
                                    >
                                  ...
                                  <td className="py-3 px-5 text-center">
                                    <span
                                      className={`px-2.5 py-1 rounded-sm text-[11px] font-bold tracking-wide ${passed ? "bg-[#dcfce7] text-[#16a34a]" : "bg-[#fee2e2] text-red-500"}`}
                                    >
                ...
                (isLoadingQuizzes ? (
                  <div className="flex flex-col gap-4">
                    {[1, 2].map((i) => (
                      <div
                        key={i}
                        className="animate-pulse bg-white h-[90px] rounded-md border border-[#e7dff0]"
                      ></div>
                    ))}
                  </div>
                ) : quizzes.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-20 px-4 bg-white border border-dashed border-[#e7dff0] rounded-md w-full text-center">
                    ...
                    <p className="text-[15px] font-bold text-[#4b3f68]">
                      No Quiz Yet
                    </p>
                    <p className="text-[13px] text-[#64748b] mt-1">
                    ...
                    {quizzes.map((quiz) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-4">
                          <span
                            className={`px-2.5 py-1 rounded-sm text-[11px] font-bold tracking-wide ${quiz.is_published ? "bg-[#dcfce7] text-[#16a34a]" : "bg-[#f1f5f9] text-[#64748b]"}`}
                          >
      ...
      <TeacherGradeModal
        ...
        onGraded={() =>
          selectedAssignment && loadSubmissionsForAssignment(selectedAssignment)
        }
      />
