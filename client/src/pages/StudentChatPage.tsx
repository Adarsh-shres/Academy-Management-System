...
export default function StudentChatPage() {
  ...
  const loadChannelsAndDMs = async () => {
    try {
      ...
      // Fetch classes where student_ids contains current user
      ...
      let teacherIds = [...new Set(classesData?.map((c: any) => c.teacher_id).filter(Boolean) || [])]
      
      // Also include anyone the student has exchanged DMs with
      ...
      if (dmHistory) {
        dmHistory.forEach(dm => {
          if (dm.sender_id !== user.id) teacherIds.push(dm.sender_id);
          if (dm.receiver_id !== user.id) teacherIds.push(dm.receiver_id);
        });
      }
      
      teacherIds = [...new Set(teacherIds)];
      
      console.log('[Chat] unique teacher ids for student DM list:', teacherIds);

      if (teacherIds.length > 0) {
        let teacherData = await fetchUsersInChunks(teacherIds);

        console.log('[Chat] teacherData:', teacherData)

        if (teacherData?.length) {
          setDmList(teacherData)
        } else {
          // Fallback: create placeholder entries from class data
          const placeholders = classesData?.map((c: any) => ({
            id: c.teacher_id,
            full_name: c.teacher_name || 'Teacher',
            email: ''
          })) || []
          setDmList([...new Map(placeholders.map(p => [p.id, p])).values()])
        }
      } else {
        setDmList([])
      }
  ...
  const fetchChannelMessages = async (channelId: string) => {
    try {
      ...
      // First try from channelMembers already fetched
      channelMembers.forEach(m => { sendersMap[m.id] = m })
      ...
      messagesData?.forEach(m => {
        if (m.content === `[SYSTEM_BLOCK:${currentUser.id}]`) {
          currentlyBlocked = true;
        } else if (m.content === `[SYSTEM_UNBLOCK:${currentUser.id}]`) {
  ...
  const fetchDMMessages = async () => {
    ...
    const allDmMessages = data?.map(m => ({
      ...m,
      sender: {
        id: m.sender_id,
        full_name: getSenderNameFallback(sendersMap[m.sender_id], currentUser?.id, m.sender_id)
    ...
    const filteredDmMessages = allDmMessages.filter(
      m => (m.sender_id === currentUser.id && m.receiver_id === selectedDM.id) ||
           (m.sender_id === selectedDM.id && m.receiver_id === currentUser.id)
    );

    let currentlyBlocked = false;
    filteredDmMessages.forEach(m => {
      if (m.content === `[SYSTEM_BLOCK:${currentUser.id}]`) {
        currentlyBlocked = true;
      } else if (m.content === `[SYSTEM_UNBLOCK:${currentUser.id}]`) {
        currentlyBlocked = false;
    ...
    const realDMs = filteredDmMessages.filter(m =>
      !m.content?.startsWith('[SYSTEM_BLOCK:') && !m.content?.startsWith('[SYSTEM_UNBLOCK:')
    );

    setDmMessages(realDMs)
    ...
    await supabase
      ...
      .update({ is_read: true })
      .eq('receiver_id', currentUser.id)
      .eq('sender_id', selectedDM.id)

    fetchUnreadCounts()
  }

  const sendDM = async () => {
    if (!dmInput.trim() || !selectedDM || !currentUser) return
  ...
  // Poll DM messages when modal is open
  useEffect(() => {
    if (showDMModal && selectedDM) {
      fetchDMMessages()
      const interval = setInterval(fetchDMMessages, 5000)
      return () => clearInterval(interval)
    }
  }, [showDMModal, selectedDM])
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (selectedDM) await fetchDMMessages();
    } catch (err: any) {
  ...
  return (
    <div className="flex h-screen bg-white overflow-hidden">
          ...
          <div className="px-4 py-4 border-t border-[#e7dff0]">
            ...
            <div className="flex flex-col gap-1">
              {filteredDMs.length === 0 ? (
                <p className="text-[12px] text-[#94a3b8]">No teachers found</p>
              ) : (
                filteredDMs.map(dm => (
                  <button
                    key={dm.id}
                    ...
                    className="text-left px-3 py-2 rounded-lg text-[13px] flex items-center gap-2 transition-colors border-transparent text-[#64748b] hover:bg-[#ede9f5]/50 border-l-2"
                  >
      ...
      {/* Chat Area */}
      <div className={`flex-1 flex-col min-w-0 ${!(selectedChannel || (selectedDM && !showDMModal)) ? 'hidden md:flex' : 'flex'}`}>
        {(selectedChannel || (selectedDM && !showDMModal)) ? (
          <>
            {/* Header */}
                  ...
                  {selectedChannel ? (
                    ...
                    <>
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-[#94a3b8] font-semibold">Teacher</p>
                      </div>
                    </>
                  )}
                </div>
            ...
            {isBlocked ? (
              <div className="border-t border-[#e7dff0] bg-white p-4 text-center">
                <p className="text-[#e11d48] font-semibold text-[13px]">You have been blocked from sending messages.</p>
              </div>
            ) : (
              <div className="border-t border-[#e7dff0] bg-white p-4">
                ...
                <div className="relative flex gap-3 items-end">
                    ...
                    <div className="absolute bottom-full left-0 mb-2 bg-white border border-[#e7dff0] rounded-xl shadow-lg flex flex-col w-64 overflow-hidden z-10">
                      {mentionSuggestions.map((user) => (
                        ...
                        >
                          ...
                          <span className="text-[13px] font-medium text-[#4b3f68] truncate">{user.full_name}</span>
                        </button>
                      ))}
                    </div>
                  )}
                  ...
                  <button
                    ...
                    className="px-3 py-2 text-[#6a5182] hover:bg-[#f6f2fb] rounded-xl transition-colors font-bold text-lg flex items-center justify-center"
                    title="Attach file"
                  >
                    <Paperclip size={20} />
                  </button>
                ...
                <div key={teacher.id} className="flex items-center justify-between group">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="min-w-0">
                      <p className="text-[13px] font-semibold text-[#4b3f68] truncate">{teacher.full_name}</p>
                ...
                {channelMembers.filter(m => m.id !== selectedChannel.teacher_id).map(student => (
                  <div key={student.id} className="flex items-center justify-between group">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <p className="text-[13px] font-medium text-[#4b3f68] truncate">{student.full_name}</p>
                    </div>
                    {currentUser?.id !== student.id && (
                      <button
                        ...
                        className="hidden group-hover:block text-[11px] text-[#6a5182] font-semibold bg-[#ede9f5] px-2 py-1 rounded shrink-0 ml-2"
                      >
                        Message
                      </button>
                    )}
      ...
      {showDMModal && selectedDM && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col" style={{ height: '70vh' }}>
            {/* Header */}
            <div className="flex items-center gap-3 p-4 border-b border-[#e7dff0]">
              <div className="w-9 h-9 rounded-full bg-[#6a5182] text-white flex items-center justify-center text-sm font-bold">
                {selectedDM.full_name?.split(' ').map((n: string) => n[0]).join('').slice(0, 2).toUpperCase()}
              </div>
              <div>
                <p className="font-semibold text-gray-800">{selectedDM.full_name}</p>
                <p className="text-xs text-gray-400">Direct Message</p>
              </div>
              <button onClick={() => { setShowDMModal(false); setSelectedDM(null); }} className="ml-auto text-gray-400 hover:text-gray-600 text-xl">✕</button>
            </div>

            {/* Messages */}
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
              {dmMessages.map(msg => (
                <div key={msg.id} className={`flex ${msg.sender_id === currentUser?.id ? 'justify-end' : 'justify-start'} gap-2`}>
                  {msg.sender_id !== currentUser?.id && (
                    <div className="w-8 h-8 rounded-full bg-purple-200 text-[#6a5182] flex items-center justify-center text-xs font-bold flex-shrink-0">
                      {(msg.sender?.full_name || 'UN').split(' ').map((n: string) => n[0]).join('').slice(0, 2).toUpperCase()}
                    </div>
                  )}
                  <div className={`max-w-xs rounded-2xl px-4 py-2 ${msg.sender_id === currentUser?.id ? 'bg-[#6a5182] text-white rounded-br-sm' : 'bg-gray-100 text-gray-800 rounded-bl-sm'}`}>
                    {msg.sender_id !== currentUser?.id && (
                      <p className="text-xs font-semibold text-[#6a5182] mb-1">{msg.sender?.full_name}</p>
                    )}
                    <p className="text-sm">{msg.content}</p>
                    <p className={`text-xs mt-1 ${msg.sender_id === currentUser?.id ? 'text-purple-200' : 'text-gray-400'}`}>
            ...
            {/* Input */}
            {isBlocked ? (
              <div className="p-4 border-t border-[#e7dff0] bg-white text-center">
                <p className="text-[#e11d48] font-semibold text-[13px]">You have been blocked from sending messages.</p>
              </div>
            ) : (
              <div className="p-4 border-t border-[#e7dff0] flex gap-2">
                <input
                  value={dmInput}
                  onChange={e => setDmInput(e.target.value)}
                  onKeyDown={e => e.key === 'Enter' && !e.shiftKey && sendDM()}
                  placeholder={`Message ${selectedDM.full_name}...`}
                  className="flex-1 border border-[#e7dff0] rounded-xl px-4 py-2 bg-[#f6f2fb] text-sm outline-none focus:border-[#6a5182]"
                />
                <button
                  onClick={sendDM}
                  disabled={!dmInput.trim()}
                  className="bg-[#6a5182] text-white rounded-xl px-4 py-2 disabled:opacity-50 hover:bg-[#4b3f68] transition"
                >
                  ➤
                </button>
              </div>
