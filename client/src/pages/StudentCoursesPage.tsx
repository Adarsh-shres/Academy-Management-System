  ...
  return (
    <div className="flex flex-col gap-8 pb-10 flex-1 min-w-0 max-w-[1100px] mx-auto w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="relative overflow-hidden rounded-[10px] border border-[#E1E6EE] bg-white p-5 shadow-[0_2px_12px_rgba(36,37,41,0.04)] flex flex-col items-center justify-center text-center">
        ...
        <div className="relative overflow-hidden rounded-[10px] border border-[#E1E6EE] bg-white p-5 shadow-[0_2px_12px_rgba(36,37,41,0.04)] flex flex-col items-center justify-center text-center">
          <p className="font-sans text-[30px] font-bold text-[#232529] leading-none tracking-tight mb-2">
            {recordedSessions}
          </p>
      ...
      {courses.length > 0 ? (
        ...
        <div className="rounded-[10px] border border-dashed border-[#E1E6EE] bg-white p-10 text-center shadow-[0_2px_12px_rgba(36,37,41,0.04)]">
          <h3 className="text-[16px] font-bold text-[#232529]">No Courses Assigned</h3>
      ...
      {selectedCourse && createPortal(
        ...
        >
          <div
            className="bg-white rounded-[10px] shadow-[0_24px_70px_rgba(15,23,42,0.22)] p-0 w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 relative border border-[#E1E6EE]"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Modal Header */}
            <div className="px-7 pt-7 pb-6 border-b border-[#F6F8FB] bg-white relative">
               <button
                onClick={() => setSelectedCourse(null)}
                className="absolute top-6 right-6 w-8 h-8 rounded-[8px] bg-white border border-[#E1E6EE] flex items-center justify-center text-[#7c8697] hover:text-[#232529] hover:shadow-sm transition-all"
               >
               ...
               <div className="flex items-start gap-4 pr-10">
                 ...
                 <div className="min-w-0">
                   <h3 className="font-sans text-[22px] font-bold text-[#232529] tracking-tight leading-tight">{selectedCourse.name}</h3>
              ...
              {courseDetailRows.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {courseDetailRows.map(({ label, value }) => (
                    <div key={label} className="rounded-[8px] border border-[#E1E6EE] bg-[#fdfcff] px-4 py-3">
                      <span className="text-[10.5px] font-bold text-[#778196] uppercase tracking-[0.08em]">{label}</span>
                      <p className="mt-1 text-[13.5px] font-semibold text-[#232529] leading-snug">{value}</p>
                    </div>
                  ))}
                </div>
              ) : (
                <div className="rounded-[8px] border border-dashed border-[#E1E6EE] bg-[#fdfcff] px-5 py-8 text-center">
                  <p className="text-[13px] font-semibold text-[#232529]">No verified course details are available yet.</p>
